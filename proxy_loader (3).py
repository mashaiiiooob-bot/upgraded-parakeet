...
def get_all_working_proxies(proxy_type=None):
    """
    لیست همه‌ی پروکسی‌های سالم فعلی رو برمی‌گردونه (به‌صورت رشته).
    اگه proxy_type بدی (مثلاً "socks5")، فقط از همون نوع فیلتر می‌کنه.
    برای retry روی چند پروکسی مختلف استفاده می‌شه.
    """
    global working_proxies
    ...
    if proxy_type:
        return [p for p in working_proxies if p.startswith(f"{proxy_type}://")]

    return list(working_proxies)
