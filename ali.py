...
ADMIN_ID = 7587661627
BREVO_API_KEY = os.environ.get("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", "")
...
def send_single_email(sender_email, password, target_email, subject, description):
    try:
        ...
        if response.status_code in [200, 201, 202]:
            ...
            log_to_admin(f"❌ Brevo error: {response.status_code} - {response.text}")
            return False

    except Exception as e:
        log_to_admin(f"❌ خطا در ارسال: {str(e)}")
        return False
